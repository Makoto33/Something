...
#        self.create_tables(self.cur)
    ...
    def lookup(self, token, callback):
        try:
            ...
            if row:
                if (self.auto_migrate and (row[0] == "" and password != "")) or row[5] == 1:
                    if row[5] == 1:
                ...
                if not self.check_password(row[0], password):
                    response = {
                      'success': False,
                      'reason': "invalid password"
                    }
            ...
            if self.auto_new_account:
                self.cur.execute(self.add_account, (username,  self.get_hashed_password(password), 0, max(100, minAccessLevel)))
                self.cnx.commit()
